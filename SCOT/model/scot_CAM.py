...
class SCOT_CAM:
    ...
    def __init__(self, backbone, hyperpixel_ids, benchmark, device, cam):
        ...
        if backbone == 'resnet50':
            ...
            raise KeyError("GluonCV not imported for fcn101")
    ...
    def __call__(self, *args, **kwargs):
        ...
        trg_mask = args[10]
        backbone = args[11]
        src_hyperpixels, hyper_h, hyper_w = self.extract_hyperpixel(args[0], maptype, src_bbox, src_mask, backbone)
        trg_hyperpixels, hyper_h, hyper_w = self.extract_hyperpixel(args[1], maptype, trg_bbox, trg_mask, backbone)
        confidence_ts = rhm_map.rhm(src_hyperpixels, trg_hyperpixels, self.hsfilter, args[2], args[3], args[4], args[5])
        return confidence_ts, src_hyperpixels[0], trg_hyperpixels[0], hyper_h, hyper_w
    ...
    def extract_hyperpixel(self, img, maptype, bbox, mask, backbone="resnet101"):
        ...
        hpgeometry = geometry.receptive_fields(rfsz, jsz, hyperfeats.size(), self.device).to(self.device)
        hyperfeats_h, hyperfeats_w = hyperfeats.size()[1], hyperfeats.size()[2] 
        ...
        return (hpgeometry, hyperfeats, img.size()[1:][::-1], weights), hyperfeats_h, hyperfeats_w
